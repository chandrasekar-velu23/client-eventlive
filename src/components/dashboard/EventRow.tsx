  ...
  return (
    <div className="flex items-center justify-between rounded-xl p-3 hover:bg-surface-50 transition-colors group border border-transparent hover:border-surface-200">
      <div className="flex items-center gap-4 flex-1 min-w-0 mr-2">
        <div className={`flex h-10 w-10 items-center justify-center rounded-lg shadow-sm shrink-0 ${live ? 'bg-red-50 text-red-600 ring-1 ring-red-100' : 'bg-surface-100 text-muted ring-1 ring-surface-200'}`}>
          <VideoCameraIcon className="h-5 w-5" />
        </div>

        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <p className="text-sm font-bold text-text-primary group-hover:text-brand-600 transition-colors truncate">
              {title}
            </p>
            {live && (
              <span className="flex h-2 w-2 rounded-full bg-red-500 animate-pulse ring-2 ring-red-100 shrink-0" />
            )}
          </div>
